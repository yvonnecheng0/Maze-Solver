""""
Module generates a maze and its solution path using a greedy algorithm given the width and height desired 
Yvonne Cheng 
csci 112
Winter, 2023
"""
...
def adjacency_list(removed_walls, maze_size):
    #Initialize adjacency dict with empty lists for each cell
    predecessor = {cell: [] for cell in range(maze_size[0] * maze_size[1])}

    #Loop through removed walls and add edges to list
...
def manhattan_dist(cell1, cell2):
    #Calculate Manhattan distance btw two cells
    return abs(cell1[0] - cell2[0]) + abs(cell1[1] - cell2[1])

def heuristic(cell, maze_size):
    #Calculate Manhattan distance from cell to goal
    end = maze_size[0] * maze_size[1] - 1
    end_pos = position(end, maze_size)
    cell_pos = position(cell, maze_size)
    return manhattan_dist(cell_pos, end_pos)

def zero_heuristic(cell, maze_size):
    return 0

def greedy(predecessor, heuristic, maze_size):
    ...
    path = []
    count = 0
    queue = [(heuristic(start, maze_size), start, [start])]

    #If there are still cells to explore
    while queue:
        _, curr_cell, curr_path = heapq.heappop(queue)

        #Base case: if current cell is end cell, solution found
        if curr_cell == end:
            path = curr_path
            break

        #Add current cell to visited set
        viseted.add(curr_cell)

        #Reverse list of neighbors for current cell and explore cells in top-left to bottom-right order
        neighbors = predecessor[curr_cell][::-1]

        #Loop through adjacent cells
        for neighbor in neighbors:
            #If neighbor not been visited, add to queue with current path and heuristic value priority
            if neighbor not in viseted:
                count += 1
                heapq.heappush(queue, (heuristic(neighbor, maze_size), neighbor, curr_path + [neighbor]))

    return path, count

def draw_maze(remaining_walls, maze_size, solution=[]):
    #Set rows and cols of maze = maze_size
    nrows, ncols = maze_size
...
if __name__ == "__main__":
    maze_sizes = [(5, 5), (10, 10), (50, 50), (75,75), (100,100)]
    for maze_size in maze_sizes:
        ...
        #Solve maze using predecessor list with heuristic
        solution, h_count = greedy(predecessor, heuristic, maze_size)

        #Solve maze using predecessor list without heuristic
        solution, count = greedy(predecessor, zero_heuristic, maze_size)

        print("Nodes added with heuristic:", h_count)
        print("Nodes added without heuristic:", count)
